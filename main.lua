...
library.rank = "Member"
local Wm = library:Watermark("Fate | v" .. library.version ..  " | " .. library:GetUsername() .. " | rank: " .. library.rank)
...
for i = 20,0,-1 do 
    task.wait(0.05)
    local LoadingFate = Notif:Notify("Loading Fate lib v1, please be patient.", 3, "information") -- notification, alert, error, success, information
end 
...
local Label1 = Tab1:NewLabel("", "left")--"left", "center", "right"

local Toggle1 = Tab1:NewToggle("Silent Aim", false, function(value)
...
local Tab2 = Init:NewTab("Legit")
local Section2 = Tab2:NewSection("Main")

local Label2 = Tab2:NewLabel("", "left")--"left", "center", "right"

local Toggle2 = Tab2:NewToggle("Aimbot", false, function(value)
    local vers = value and "on" or "off"
    print("one " .. vers)
end):AddKeybind(Enum.KeyCode.RightControl)
...
local Button2 = Tab2:NewButton("Button", function()
    print("one")
end)

local Keybind2 = Tab2:NewKeybind("Keybind 1", Enum.KeyCode.RightAlt, function(key)
    Init:UpdateKeybind(Enum.KeyCode[key])
end)
...
local FinishedLoading = Notif:Notify("Loaded Fate Private", 4, "success")
...
--[[
    ...
    -- / library:PromptDiscord("https://discord.gg/pt4p6pGhe7")
